const monday = [
  ...
  const workInHours = tasks.map((minutes) => minutes.duration / 60 );
  
  const greaterThan2Hours = workInHours.filter((time) => time >=2 );

  const payment = greaterThan2Hours.map((amount) => amount * 20 );
   
  const sum = payment.reduce((accumulate, value) => accumulate + value );

  const totalInEuros = "€ " + sum.toFixed(3);
